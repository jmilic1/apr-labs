...
func (m *Matrix) SizeSquare() int {
	n := len(m.elements)
	m1 := len(m.elements[0])
	if n != m1 {
		panic("n not equal to m")
	}
...
func Sub(m1 *Matrix, m2 *Matrix) *Matrix {
	return elementWise(minus, m1, m2)
}

func (m *Matrix) MulScalar(scalar float64) *Matrix {
	return MulScalar(m, scalar)
}
...
func MulScalar(m *Matrix, scalar float64) *Matrix {
	rows := m.rows
	columns := m.columns
	...
	for i := 0; i < rows; i++ {
		for j := 0; j < columns; j++ {
			...
			newMatrix.Set(i, j, element*scalar)
		}
	}
